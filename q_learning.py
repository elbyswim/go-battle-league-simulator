...
class QLearning:
    def __init__(
        ...
        self.dqn = DQN(input_dim, hidden_size, n_actions)
    ...
    def select_action(
        ...
        # if True:
        if e < epsilon:
            action = np.random.choice(torch.arange(self.n_actionsd).reshape(1, self.n_actions)[possible_actions])
        # exploitation
        else:
            with torch.no_grad():
...
class QLearningTeam:
    ...
    def select_action(
        self, state_1: torch.Tensor, state_2: torch.Tensor, team: Team, epsilon: float = 0.5
    ) -> int:
        possible_actions = team.get_actions()  # tensor[bool]
        # exploration
        e = rand(1)
        # if True:
        if e < epsilon:
            action = np.random.choice(torch.arange(self.n_actions).reshape(1, self.n_actions)[possible_actions])
        # exploitation
        else:
            with torch.no_grad():
                q_values = self.dqn(state_1, state_2)
    ...
    def initialize_teams(self, pokemon_df: pd.DataFrame, fast_moves_df: pd.DataFrame, charged_moves_df: pd.DataFrame, opponent_class):
        teams = []
        for t in range(2):
            pokemon = []
            for p in range(3):
                fast_move = FastAttack(*fast_moves_df.iloc[np.random.choice(len(fast_moves_df))].tolist())
                charged_move_1 = ChargedAttack(*charged_moves_df.iloc[np.random.choice(len(charged_moves_df))].tolist()[:4])
                charged_move_2 = ChargedAttack(*charged_moves_df.iloc[np.random.choice(len(charged_moves_df))].tolist()[:4])
                index = np.random.choice(len(pokemon_df))
                pokemon.append(
                    Pokemon(
                        *pokemon_df.iloc[index].tolist()[:6],
                        dict(zip(pokemon_df.columns[6:], pokemon_df.iloc[index].values[6:])),
                        fast_move,
                        charged_move_1,
                        charged_move_2,
                    )
    ...
    def learn(
        ...
        for epoch in trange(max_epochs):
            # print(f"################################# Epoch {epoch} ######################################")
            team_1, team_2 = self.initialize_teams(pokemon_df, fast_moves_df, charged_moves_df, opponent_class)
            battle = TeamBattle(team_1, team_2)
            state_1, state_2 = battle.get_state()
            reward_episode = 0
            for t in count():
                action_1 = self.select_action(state_1, state_2, team_1, 1 - epoch / max_epochs)
                assert team_1.get_actions()[0, action_1]
                values = self.dqn(state_1, state_2)
                value = values[0, action_1]
                action_2 = self.select_action(state_2, state_1, team_2, 1 - epoch / max_epochs)
                battle.update(action_1, action_2)
                next_state_1, next_state_2 = battle.get_state()
                reward = battle.get_reward(action_1)
                # reward_episode = reward + self.gamma * reward_episode
                ...
                with torch.no_grad():
                    next_values = self.dqn(next_state_1, next_state_2)
                next_values[~team_1.get_actions()] = -float("inf")
                expected_value = reward + self.gamma * torch.amax(next_values)
                # print(action_1, values, expected_value)
                loss = self.loss_function(value, expected_value)
                # print(action_1, action_2)
