...
def simulate_team_battle(agent: QLearning, team_1: Team, team_2: Team, max_steps: int = 600) -> None:
    ...
    for i in range(max_steps):
        ...
        # action_1 = random.choice(torch.arange(6).reshape(1, 6)[team_1.get_actions()]).item()
        action_2 = random.choice(torch.arange(6).reshape(1, 6)[team_2.get_actions()]).item()
        battle.update(action_1, action_2)

    plt.plot(hp[0], label="Team 1 Poke 1")
    plt.plot(hp[1], label="Team 1 Poke 2")
    plt.plot(hp[2], label="Team 1 Poke 3")
    plt.plot(-hp[3], label="Team 2 Poke 1")
    plt.plot(-hp[4], label="Team 2 Poke 2")
    plt.plot(-hp[5], label="Team 2 Poke 3")
    plt.legend()
    plt.show()